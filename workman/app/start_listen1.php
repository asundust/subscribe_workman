...
$text_worker = new Worker("tcp://0.0.0.0:5001");
...
$text_worker->onMessage = function ($connection, $data) use (&$isBusy) {
    ...
    $phpPath = '/usr/local/php72/bin/php';
    $path = '/data/wwwroot/' . $data['dir'];
    if (!is_dir($path)) {
    ...
    $result = shell_exec('cd ' . $path . '/ && ' . $phpPath . ' artisan queue:restart 2>&1');
    $isBusy = 0;
